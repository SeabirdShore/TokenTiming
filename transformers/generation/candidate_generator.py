...
class AssistedCandidateGeneratorDifferentTokenizers(AssistedCandidateGenerator):
    ...
    def _prepare_assistant_input_ids(self, target_input_ids: torch.LongTensor) -> torch.LongTensor:
        ...
        if hasattr(self._atm_translator, 'unmap_input_ids'):
            try:
                self._atm_translator.unmap_input_ids()
            except Exception as e:
                print(f"warning: unmap_input_ids calls error: {e}")
...
class AssistantToTargetTranslator:
    ...
    Args:
        ...
        generation_config (Optional[GenerationConfig]): Generation configuration that may contain the mapping_strategy parameter.
            When mapping_strategy is "dtw", token mapping will use DTW alignment. Defaults to None.
    """
    ...
    def __init__(
        ...
        self.use_dtw = False
        if generation_config and hasattr(generation_config, "mapping_strategy"):
            self.use_dtw = generation_config.mapping_strategy == "dtw"

        self._prev_assistant_ids = None
        self._prev_target_ids = None
        self._num_new_target_tokens = None

        if self.use_dtw:
            self._setup_dtw_mapping()
        ...
        if len(self._suppress_input_ids) > 0 and not self.use_dtw:
    ...
    def _setup_dtw_mapping(self):

        self.base_special_token = self._get_special_token_for_tokenizer(self._target_tokenizer)
        self.blending_special_token = self._get_special_token_for_tokenizer(self._assistant_tokenizer)
        self._dtw_window = 32  # DTW算法的窗口大小，可调整
    
    def _get_special_token_for_tokenizer(self, tokenizer):

        tokenizer_cls = tokenizer.__class__
        special_tokens = {
            'LlamaTokenizer': ' ',
            'PreTrainedTokenizerFast': 'Ġ',
            'GPTNeoXTokenizerFast': 'Ġ',
            'CodeGenTokenizer': 'Ġ',
            'GPT2Tokenizer': 'Ġ',
            'GemmaTokenizer': 'Ġ',
            'Qwen2Tokenizer': 'Ġ',
        ...
        for cls_name, token in special_tokens.items():
            if cls_name in str(tokenizer_cls):
                return token
        return ''
    
    def token_distance(self, a: str, b: str) -> float:

        aa = a.replace(self.blending_special_token, '')
        bb = b.replace(self.base_special_token, '')
        
        if len(aa) == 0 or len(bb) == 0:
            aa = a.replace(self.blending_special_token, self.base_special_token)
            bb = b
        
        return editdistance.eval(aa, bb)
    
    def dtw(self, series_1: List[str], series_2: List[str], window: int = None) -> Tuple[List[Tuple[int, int]], List[List[int]], List[List[int]]]:

        window = window or self._dtw_window
        len1, len2 = len(series_1), len(series_2)
        
        window = max(window, abs(len1 - len2))
        
        matrix = np.full((len1 + 1, len2 + 1), np.inf)
        matrix[0, 0] = 0
        
        for i in range(1, len1 + 1):
            min_j = max(1, i - window)
            max_j = min(len2, i + window)
            for j in range(min_j, max_j + 1):
                cost = self.token_distance(series_1[i-1], series_2[j-1])
                matrix[i, j] = cost + min(matrix[i-1, j], matrix[i, j-1], matrix[i-1, j-1])
        
        i, j = len1, len2
        matches = []
        mappings_series_1 = [[] for _ in range(len1)]
        mappings_series_2 = [[] for _ in range(len2)]
        
        while i > 0 and j > 0:
            matches.append((i-1, j-1))
            mappings_series_1[i-1].append(j-1)
            mappings_series_2[j-1].append(i-1)
    ...
    def get_target_ids_with_dtw(self, assistant_input_ids, target_input_ids, assistant_candidate_ids):
        #print(">>>get_target_ids_with_dtw<<<")
        num_new_tokens = assistant_candidate_ids.shape[1] - assistant_input_ids.shape[1]
        if num_new_tokens == 0:
            self._num_new_target_tokens = 0
            return target_input_ids
            
        new_tokens = assistant_candidate_ids[0, -num_new_tokens:].tolist()
        generated_text = self._assistant_tokenizer.decode(new_tokens, skip_special_tokens=True)
        
        target_tokens = self._target_tokenizer.encode(
            generated_text, 
            add_special_tokens=False, 
            return_tensors="pt"
        ).to(target_input_ids.device)

        self._num_new_target_tokens = target_tokens.shape[1]
        
        self._prev_assistant_ids = assistant_candidate_ids
        target_candidate_ids = torch.cat([target_input_ids, target_tokens], dim=1)
        self._prev_target_ids = target_candidate_ids

        return target_candidate_ids
    
    def get_target_logits_with_dtw(self, assistant_logits, assistant_candidate_ids, target_candidate_ids):
        assistant_seq_len = assistant_candidate_ids.shape[1]
        target_seq_len = target_candidate_ids.shape[1]
        assistant_prompt_len = assistant_seq_len - assistant_logits.shape[1]
        
        if self._num_new_target_tokens is not None:
            target_prompt_len = target_seq_len - self._num_new_target_tokens
        
        if assistant_logits.shape[1] == 0 or self._num_new_target_tokens == 0:
            return None, None
        
        assistant_tokens = [
            self._assistant_tokenizer.decode([id]) 
            for id in assistant_candidate_ids[0, assistant_prompt_len:].tolist()
        ]
        target_tokens = [
            self._target_tokenizer.decode([id]) 
            for id in target_candidate_ids[0, target_prompt_len:].tolist()
        ]
        
        matches, _, target_to_assistant_indices = self.dtw(assistant_tokens, target_tokens)
        
        target_logits_shape = (assistant_logits.shape[0], len(target_tokens), self.target_vocab_size)
        target_logits = torch.full(
            target_logits_shape, self.FILTER_VALUE, device=self._assistant_model_device
        )
        
        for target_idx, assistant_indices in enumerate(target_to_assistant_indices):
            if assistant_indices:  
                target_token_id = target_candidate_ids[0, target_prompt_len + target_idx].item()
                
                for asst_idx in assistant_indices:
                    if asst_idx < assistant_logits.shape[1]:

                        target_logits[:, target_idx, target_token_id] = 0

                        max_logit_value = assistant_logits[:, asst_idx].max().item()

                        target_logits[:, target_idx, target_token_id] = max_logit_value  
        return target_logits, target_to_assistant_indices
    ...
    def unmap_input_ids(self):
        ...
        """
        if self.assistant_prune_lm_head and hasattr(self, 'map_input_embeddings'):
    ...
    def get_target_ids(
        ...
        """
        if self.use_dtw:
            return self.get_target_ids_with_dtw(assistant_input_ids, target_input_ids, assistant_candidate_ids)
    ...
    def get_target_logits(self, assistant_logits: torch.FloatTensor) -> torch.FloatTensor:
        ...
        """
        if (self.use_dtw) and hasattr(self, "_prev_assistant_ids") and hasattr(self, "_prev_target_ids"):
            dtw_logits, target_to_assistant_indices = self.get_target_logits_with_dtw(
                assistant_logits, self._prev_assistant_ids, self._prev_target_ids
            )
            if dtw_logits is not None:
                return dtw_logits
...
class UniversalSpeculativeDecodingGenerator(AssistedCandidateGeneratorDifferentTokenizers):
    ...
    def _prepare_assistant_input_ids(self, target_input_ids: torch.LongTensor) -> torch.LongTensor:
        ...
        # 安全地调用unmap_input_ids方法
        if hasattr(self._atm_translator, 'unmap_input_ids'):
            try:
                self._atm_translator.unmap_input_ids()
            except Exception as e:
                print(f"警告: unmap_input_ids调用出错: {e}")
